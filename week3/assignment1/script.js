...
const {
    ...
    payment: { total, currency, details: { subtotal, tax, shipping }, transactions }
} = user
...
const userInfoHTML = `
  ...
  <p>${user.address.shipping.city}, ${user.address.shipping.state} ${user.address.shipping.postalCode}</p>
  <p>${user.address.shipping.country}</p>
  <h3>Billing Address</h3>
  <p>${user.address.billing.street}</p>
  <p>${user.address.billing.city}, ${user.address.billing.state} ${user.address.billing.postalCode}</p>
